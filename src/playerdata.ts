...
/**
   ...
   *
  */
export let modPlayerData:{ [index:number] : PlayerData }={
}
...
export function GetPlayerId(player:EntityPlayer|null){
  ...
  for(let i=0;i<constants.game.GetNumPlayers();i++){
    let playeri=constants.game.GetPlayer(i);
    if(playeri!=null && player !=null && playeri.GetCollectibleRNG(1).GetSeed()==player.GetCollectibleRNG(1).GetSeed()){
      returner = i;
    }
  }
  return returner;
